...
paths <- here("Plots Protocol")
#ggsave("all_deaths_log.png", path = paths)
```
...
geom_point(alpha = 0.7, size = 3, position = position_jitter(width = 0.2, height = 0)) +
labs(title = "All Deaths by Age Group and Education - normalized",
       x = "Ten-Year Age Group",
       y = "log(Number of Deaths)") +
  scale_color_brewer(palette = "Dark2") +
  theme_minimal() +
...
#ggsave("all_deaths_log_nrom.png", path = paths)
```
...
```{r Show overdispersion}
poisson_model <- glm(Deaths ~ Ten.Year.Age.Groups + Sex + Education + offset(log(adjusted_population)),
                     data = all_deaths,
...
deathrates_ref <- ggplot(edu_coef, aes(x = education_level, y = rr)) +
  geom_point(size = 2) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
deathrates_ref
#ggsave("deathrates_ref.png", path = paths)
```
...
Age25vs85 <-  ggplot(plot_data_age, aes(x = education_level, y = rr, color = age_group)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
#ggsave("Age25vs85.png", path = paths)
```
...
#ggsave("marginal_effects.png", path = paths)



emmeans(nb_stroke_int, ~ Education | Sex, type = "response")
emmeans(nb_covid_int, ~ Education | Sex, type = "response")
emmeans(nb_neoplasm_int, ~ Education | Sex, type = "response")
emmeans(nb_heart_int, ~ Education | Sex, type = "response")
emmeans(nb_accidents_int, ~ Education | Sex, type = "response")

emmeans(nb_stroke_int, ~ Education, type = "response")
emmeans(nb_covid_int, ~ Education, type = "response")
emmeans(nb_neoplasm_int, ~ Education, type = "response")
emmeans(nb_heart_int, ~ Education, type = "response")
emmeans(nb_accidents_int, ~ Education, type = "response")



stroke_data %>% filter(Cause == "Cerebrovascular Diseases") %>% filter(Sex == "Female") %>% mutate(a = Deaths / adjusted_population) %>% pull(a) %>% sum()
stroke_data %>% filter(Cause == "Cerebrovascular Diseases") %>% filter(Sex == "Male") %>% mutate(a = Deaths / adjusted_population) %>% pull(a) %>% sum()
```
